//Error Messages
const $nameError = $('<p><span class="error">Please enter your name</span></p>').insertAfter($('#name'));
const $emailErrorRealTime = $('<p><span class="error">Please provide a valid address</span></p>').insertAfter($('#mail'));
const $actError = $('<p><span class="error">Please select an activity</span></p>').insertAfter($('.activities'));
const $paymentError = $('<p><span class="error">Please select a payment method</span></p>').insertAfter($('#payment'));
const $ccnumError = $('<p><span class="error">Please enter a credit card number (omit letters)</span></p>').insertAfter($('#cc-num'));
const $ccnumError2 = $('<p><span class="error">Please enter a number 13 to 16 digits long</span></p>').insertAfter($('#cc-num'));
const $zipError = $('<p><span class="error">Please provide a zip code</span></p>').insertAfter($('#zip'));
const $cvvError = $('<p><span class="error">Please provide a 3 digit cvv</span></p>').insertAfter($('#cvv'));
$nameError.hide();
...
(function($){
    $.fn.ccnumValidator = function($param){
        if(!$($param).val().match(ccnumRegex) && $($param).val().length >0 && $($param).val().length < 13 || $($param).val().length > 16){
            $ccnumError.hide();
            $ccnumError2.show();
            $($param).addClass('red-border');
            return false;
...
let $tempColorOption = $('#color').prepend($('<option>').val('colorPreReq').text('Please select a T-shirt theme'));
...
$('#design').on('change', function(){
    if($(this).val() ==='select theme'){
        ...
        $tempColorOption = $('#color').prepend($('<option>').val('colorPreReq').text('Please select a T-shirt theme')).val('select theme').attr('selcted', '');
        $("#color option[value='colorPreReq']").attr('selected', '');
        $tempColorOption.attr('disabled', '');
        $('#colors-js-puns').hide();
...
//activities section
$('.activities').append('<strong><p id="costDisplay">Total: $'+totalActivityCost +'<p></strong>');
$('input:checkbox').on('change', function(){
    ...
    if($(this).prop('checked')){
        ...
        $('input:checkbox:not(:checked)').each(function(){
                $(this).attr('data-day-and-time') == $dayAndTime ? $(this).prop('disabled', true): "";
        });
        totalActivityCost+=activityCostRawNumber;
    } else if ($(this).prop('checked', false)){
        $('input:checkbox:not(:checked)').each(function(){
            $(this).attr('data-day-and-time') == $dayAndTime ? $(this).prop('disabled', false): "";
        });
        totalActivityCost-=activityCostRawNumber;
    }
...
$('form').on('submit', function(e){
    ...
    if (nameResult == false || mailResult == false || ccnumResult == false || zipResult == false || cvvResult == false){
        (e).preventDefault();
